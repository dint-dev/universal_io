...
class InternetAddress {
  ...
  static final InternetAddress loopbackIPv4 = InternetAddress("");
  ...
  static final InternetAddress loopbackIPv6 = InternetAddress("");
  ...
  Future<InternetAddress> reverse() {
    return IODriver.current.reverseLookupInternetAddress(this);
  }
  ...
  static Future<List<InternetAddress>> lookup(String host,
    ...
    return IODriver.current.lookupInternetAddresses(host, type: type);
  }
}
...
abstract class NetworkInterface {
  ...
  static Future<List<NetworkInterface>> list(
      ...
      InternetAddressType type = InternetAddressType.any}) {
    return IODriver.current.listNetworkInterfaces(
...
abstract class RawServerSocket implements Stream<RawSocket> {
  ...
  static Future<RawServerSocket> bind(address, int port,
    ...
    return IODriver.current.bindRawServerSocket(
      address,
      port,
      backlog: backlog,
      v6Only: v6Only,
...
abstract class ServerSocket implements Stream<Socket> {
  ...
  static Future<ServerSocket> bind(address, int port,
      {int backlog = 0, bool v6Only = false, bool shared = false}) {
    return IODriver.current.bindServerSocket(
      address,
      port,
      backlog: backlog,
      v6Only: v6Only,
...
class RawSocketOption {
  ...
  static int get levelSocket => throw UnimplementedError();

  /// Socket level option for IPPROTO_IP.
  static int get levelIPv4 => throw UnimplementedError();

  /// Socket option for IP_MULTICAST_IF.
  static int get IPv4MulticastInterface => throw UnimplementedError();

  /// Socket level option for IPPROTO_IPV6.
  static int get levelIPv6 => throw UnimplementedError();

  /// Socket option for IPV6_MULTICAST_IF.
  static int get IPv6MulticastInterface => throw UnimplementedError();

  /// Socket level option for IPPROTO_TCP.
  static int get levelTcp => throw UnimplementedError();

  /// Socket level option for IPPROTO_UDP.
  static int get levelUdp => throw UnimplementedError();
}
...
abstract class RawSocket implements Stream<RawSocketEvent> {
  ...
  static Future<ConnectionTask<RawSocket>> startConnect(host, int port,
    ...
    return IODriver.current
        .startConnectRawSocket(host, port, sourceAddress: sourceAddress);
  }
